...
import {
  ...
  IconMicrophone, IconPlayerStop, IconLoader
...
export function AICopilotWidget() {
  ...
  const { voiceState, toggleRecording, voiceError, clearVoiceError } = useVoiceChat({
    onTranscript: (transcript) => {
      // Langsung kirim transcript ke AI seolah-olah user mengetik
      ...
      <Transition transition="slide-up" mounted={isWidgetOpen}>
        {(styles) => (
          ...
          >
            ...
            {/* VOICE INDICATOR */}
            {voiceState !== 'idle' && (
              <Box px="sm" py={6} style={{ borderTop: '1px solid #e9ecef', background: voiceState === 'recording' ? '#fff5f5' : '#f0fffe' }}>
            ...
            {/* VOICE ERROR */}
            {voiceError && (
              <Box px="sm" py={4} style={{ borderTop: '1px solid #e9ecef', background: '#fff5f5' }}>
