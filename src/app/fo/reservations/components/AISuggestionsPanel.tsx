...
import { 
  ...
  IconInfoCircle, IconFileInvoice, IconMicrophone, IconPlayerStop
...
const RESERVATION_QUICK_PROMPTS = [
  {
    ...
    color: 'teal',
    desc: 'Buat reservasi langsung'
  },
  {
    label: 'Cek Ketersediaan',
    text: 'Cek ketersediaan kamar [Tipe Kamar] untuk tanggal [Check-in] sampai [Check-out]',
    icon: <IconBed size={14} />,
    color: 'blue',
  ...
  },
  {
    ...
    text: 'Berikan rekomendasi penawaran atau upgrade kamar untuk tamu [Nama Tamu]',
    icon: <IconBulb size={14} />,
    color: 'grape',
    desc: 'Rekomendasi penawaran'
  }
];

// Komponen untuk Konfirmasi Booking
  ...
  return (
    <Card shadow="sm" padding="md" radius="md" withBorder>
      <Group justify="space-between" mb="sm">
        ...
        <Badge color="cyan" variant="light">{data.summary.total_rooms} Kamar</Badge>
      </Group>
...
export function AICoPilotPanel() {
  ...
  const renderMessageContent = (msg: ChatMessage, index: number) => {
    ...
    if (msg.responseType === 'room_status' && msg.data) {
      return <RoomStatusCard data={msg.data} />;
    }
  ...
  return (
    <Stack gap="md" h="100%" style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          {chatHistory.length === 0 && (
            <Box>
              <Box style={{ textAlign: 'center', opacity: 0.8, marginBottom: 20 }}>
                ...
                <Text fw={600} size="md">Halo, Reservation Team!</Text>
                <Text size="xs" c="dimmed">
                  Pilih menu cepat atau ketik perintah Anda
                </Text>
              </Box>
      ...
      {/* Voice Indicator */}
      {voiceState !== 'idle' && (
        <Box px="sm" py={6} style={{ background: voiceState === 'recording' ? '#fff5f5' : '#f0fffe', borderRadius: 8 }}>
          <Group gap={6} justify="center">
              ...
              <>
                <Box
                  style={{
                    ...
                    animation: 'pulse-recording-res 1.2s ease-in-out infinite',
                  }}
                />
            ...
            {voiceState === 'processing' && (
              <>
                ...
                <Text size="xs" c="teal.7" fw={500}>Memproses suara...</Text>
              </>
            )}
          </Group>
        </Box>
        ...
        <Group gap="xs" align="flex-end">
          <Tooltip label={voiceState === 'recording' ? 'Berhenti merekam' : voiceState === 'processing' ? 'Memproses...' : 'Bicara ke AI'}>
            <ActionIcon
              ...
              color={voiceState === 'recording' ? 'red' : 'teal'}
              ...
              style={{
                ...(voiceState === 'recording' ? {
                  ...
                  animation: 'pulse-idle-mic 2s infinite',
                }),
      ...
      <style>{`
        @keyframes pulse-recording-res {
          0%, 100% { opacity: 1; transform: scale(1); }
          50% { opacity: 0.6; transform: scale(1.05); }
        }
        @keyframes pulse-idle-mic {
...
// Export juga AISuggestionsPanel untuk backward compatibility jika ada yang pakai
