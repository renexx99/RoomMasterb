...
import {
  ...
  Stack, Paper, ActionIcon, Text, Box, Loader, Center, Select,
  Badge, NumberInput, Grid, MultiSelect,
} from '@mantine/core';
...
import {
    ...
    IconBed, IconCalendar, IconCash, IconSearch
...
import { Guest, Reservation, Room, RoomType, PaymentStatus } from '@/core/types/database';
...
function ReservationsManagementContent() {
  ...
  const fetchGuestsForSelect = async () => {
     if (!assignedHotelId) return;
    const { data, error } = await supabase.from('guests').select('id, full_name, email').eq('hotel_id', assignedHotelId).order('full_name', { ascending: true }); // Gunakan assignedHotelId
    if (error) throw error;
  ...
  const fetchAvailableRooms = async () => {
     if (!assignedHotelId) return;
     const { data, error } = await supabase.from('rooms').select(`*, room_type:room_types(*)`).eq('hotel_id', assignedHotelId).eq('status', 'available').order('room_number', { ascending: true }); // Gunakan assignedHotelId
     if (error) throw error;
  ...
  const filteredAndSortedReservations = useMemo(() => {
    // ... (Logika filter dan sort sama persis seperti admin/reservations)
    ...
    switch (sortBy) {
        ...
        case 'guest_name_desc': result.sort((a, b) => (b.guest?.full_name || '').localeCompare(a.guest?.full_name || '')); break;
        // ... (semua case sort lainnya sama)
        case 'check_in_asc': default: result.sort((a, b) => new Date(a.check_in_date).getTime() - new Date(b.check_in_date).getTime()); break;
  ...
  const handleSubmit = async (values: typeof form.values) => {
    ...
    try {
      ...
      if (guestSelectionMode === 'new') {
        const { data: newGuest, error: guestError } = await supabase.from('guests').insert({ hotel_id: assignedHotelId, full_name: values.new_guest_name, email: values.new_guest_email, phone_number: values.new_guest_phone || null }).select('id').single(); // Gunakan assignedHotelId
      ...
      const reservationData = { hotel_id: assignedHotelId, guest_id: targetGuestId, room_id: values.room_id, check_in_date: values.check_in_date, check_out_date: values.check_out_date, total_price: calculatedPrice ?? 0, payment_status: values.payment_status }; // Gunakan assignedHotelId
 ...
 const handleEdit = (reservation: ReservationDetails) => {
  ...
  const handleCloseModal = () => { setModalOpened(false); setEditingReservation(null); setGuestSelectionMode('select'); setCalculatedPrice(null); form.reset(); };
  const handleCloseDeleteModal = () => { setDeleteModalOpened(false); setDeleteTarget(null); };
  const guestOptions = useMemo(() => guestOptionsData.map((g) => ({ value: g.id, label: `${g.full_name} (${g.email})` })), [guestOptionsData]);
  const roomOptionsModal = useMemo(() => availableRooms.map((r) => ({ value: r.id, label: `Kamar ${r.room_number} (${r.room_type?.name || '...'}) - Rp ${r.room_type?.price_per_night?.toLocaleString('id-ID') ?? 'N/A'}/malam` })), [availableRooms]);
  const paymentStatusOptions = [ { value: 'pending', label: 'Pending' }, { value: 'paid', label: 'Paid' }, { value: 'cancelled', label: 'Cancelled' } ];
  const getPaymentStatusColor = (status: string) => { switch (status) { case 'paid': return 'green'; case 'pending': return 'orange'; case 'cancelled': return 'red'; default: return 'gray'; } };
  ...
  return (
    <DatesProvider settings={{ locale: 'id', firstDayOfWeek: 1, weekendDays: [0] }}>
            ...
            <Container size="lg">
                <Group justify="space-between" align="center">
                    <div>
                     ...
                     {/* Tombol warna biru */}
                     <Button leftSection={<IconPlus size={18} />} onClick={() => { setEditingReservation(null); setGuestSelectionMode('select'); form.reset(); setCalculatedPrice(null); setModalOpened(true); }} variant="white" color="blue" > Buat Reservasi </Button>
                </Group>
            </Container>
        </div>
        ...
        <Container size="lg" pb="xl">
            ...
            <Stack gap="lg">
                    ...
                    <Paper shadow="xs" p="md" radius="md" withBorder mb="lg">
                        <Grid align="flex-end" gutter="md">
                            <Grid.Col span={{ base: 12, md: 4 }}>
                                <TextInput label="Cari Reservasi" placeholder="Nama tamu, email, atau no. kamar..." leftSection={<IconSearch size={16} />} value={searchTerm} onChange={(event) => setSearchTerm(event.currentTarget.value)} />
                            </Grid.Col>
                             <Grid.Col span={{ base: 12, sm: 6, md: 3 }}>
                                <DatePickerInput type="range" label="Rentang Check-in" placeholder="Pilih rentang tanggal" value={filterDateRange} onChange={setFilterDateRange} clearable valueFormat="DD MMM YYYY" />
                            </Grid.Col>
                            <Grid.Col span={{ base: 12, sm: 6, md: 2 }}>
                                <MultiSelect label="Status Bayar" placeholder="Semua Status" data={paymentStatusOptions} value={filterPaymentStatus} onChange={setFilterPaymentStatus} clearable />
                            </Grid.Col>
                            <Grid.Col span={{ base: 12, md: 3 }}>
                                <Select label="Urutkan" value={sortBy} onChange={(value) => setSortBy(value || 'check_in_asc')}
                                    data={[
                ...
                {reservations.length === 0 && !loading ? (
                    <Paper shadow="sm" p="xl" radius="md" withBorder> <Box ta="center"> <Text c="dimmed" mb="md"> Belum ada reservasi. Klik tombol 'Buat Reservasi' di atas. </Text> </Box> </Paper>
                ) : filteredAndSortedReservations.length === 0 ? (
                    <Paper shadow="sm" p="lg" radius="md" withBorder> <Text c="dimmed" ta="center" py="xl"> Tidak ada reservasi yang cocok dengan filter atau pencarian Anda. </Text> </Paper>
                ) : (
        ...
        <Modal opened={modalOpened} onClose={handleCloseModal} title={editingReservation ? 'Edit Reservasi' : 'Buat Reservasi Baru'} centered size="lg" >
            ...
            <Stack gap="md">
                <Group grow> <Button variant={guestSelectionMode === 'select' ? 'filled' : 'outline'} onClick={() => setGuestSelectionMode('select')} color="blue">Pilih Tamu</Button> <Button variant={guestSelectionMode === 'new' ? 'filled' : 'outline'} onClick={() => setGuestSelectionMode('new')} color="blue">Tamu Baru</Button> </Group>
                ...
                <Group justify="flex-end" mt="md"> <Button variant="default" onClick={handleCloseModal}> Batal </Button> <Button type="submit" loading={loading} disabled={calculatedPrice === null || calculatedPrice <= 0} color="blue"> {editingReservation ? 'Update Reservasi' : 'Simpan Reservasi'} </Button> </Group>
            </Stack>
            </form>
        </Modal>
