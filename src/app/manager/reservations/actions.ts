...
export async function createReservation(data: ReservationData) {
  ...
  if (data.payment_status === 'paid' && fullReservation) {
    try {
      ...
      console.log('[RESERVATION] Awarding loyalty points:', { guest_id: data.guest_id, hotel_id: data.hotel_id, nights, total_price: data.total_price });
      const loyaltyResult = await awardStayPoints(
        data.guest_id,
        data.hotel_id,
        insertedId.id,
        nights,
      ...
      console.log('[RESERVATION] Loyalty result:', loyaltyResult);
    } catch (e: any) {
      console.error('[RESERVATION] Failed to award loyalty points:', e?.message || e);
    }
  }
