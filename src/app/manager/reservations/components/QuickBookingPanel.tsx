...
import {
  Stack, Paper, Group, Text, TextInput, Select, Button, ThemeIcon
} from '@mantine/core';
  ...
  const handleQuickBook = async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        setGuestMode('existing'); setSelectedGuest(null); setGuestName(''); setGuestPhone(''); setGuestEmail('');
        setCheckInDate(null); setCheckOutDate(null); setSelectedRoom(null); setCalculatedPrice(null);
        onSuccess();
      }
    } catch (error) {
    ...
    <Stack gap="md">
      <Paper p="md" radius="md" withBorder style={{ background: 'white' }}>
        ...
        <Stack gap="sm">
          ...
          {guestMode === 'existing' ? (
            <Select label="Pilih Tamu" placeholder="Cari tamu..." data={guestOptions} value={selectedGuest} onChange={setSelectedGuest} searchable leftSection={<IconUser size={16} />} />
          ) : (
            <>
              <Select label="Title" data={['Mr.', 'Mrs.', 'Ms.', 'Dr.', 'Prof.']} value={guestTitle} onChange={setGuestTitle} />
              <TextInput label="Nama Lengkap" placeholder="Nama tamu" value={guestName} onChange={(e) => setGuestName(e.currentTarget.value)} leftSection={<IconUser size={16} />} required />
              <TextInput label="Email" placeholder="email@example.com" value={guestEmail} onChange={(e) => setGuestEmail(e.currentTarget.value)} leftSection={<IconMail size={16} />} required />
              <TextInput label="Telepon" placeholder="+62 xxx" value={guestPhone} onChange={(e) => setGuestPhone(e.currentTarget.value)} leftSection={<IconPhone size={16} />} />
            </>
          )}
        </Stack>
      </Paper>
      ...
      <Paper p="md" radius="md" withBorder style={{ background: 'white' }}>
        ...
        <Stack gap="sm">
          <DatePickerInput label="Check-in" placeholder="Pilih tanggal" value={checkInDate} onChange={(date) => setCheckInDate(date as Date | null)} minDate={new Date()} required />
          <DatePickerInput label="Check-out" placeholder="Pilih tanggal" value={checkOutDate} onChange={(date) => setCheckOutDate(date as Date | null)} minDate={checkInDate || new Date()} required />
          <Select label="Pilih Kamar" placeholder="Kamar tersedia" data={availableRooms.map(r => ({ value: r.id, label: `${r.room_number} - ${r.room_type?.name} (Rp ${r.room_type?.price_per_night.toLocaleString('id-ID')})` }))} value={selectedRoom} onChange={setSelectedRoom} leftSection={<IconBed size={16} />} searchable required />
      ...
      <Button 
        fullWidth size="md" leftSection={<IconCheck size={18} />} onClick={handleQuickBook} loading={isSubmitting} disabled={!calculatedPrice}
        variant="gradient" gradient={{ from: '#3b82f6', to: '#2563eb', deg: 135 }}
      >
        Buat Reservasi
      </Button>
    </Stack>
