      ...
      <Stack gap="sm" style={{ flex: 1 }}>
        <Group justify="space-between" align="flex-start">
          <div style={{ flex: 1 }}>
            ...
            <Text size="xl" fw={700} c="blue" mt={4}>
              IDR {roomType.price_per_night.toLocaleString('en-US')}
              <Text span size="sm" c="dimmed" fw={400}> /night</Text>
            </Text>
          </div>
          <Group gap={4}>
        ...
        <Stack gap={6}>
          ...
          <Group gap={8}>
            ...
            <Text size="sm">Capacity: {roomType.capacity} pax</Text>
          </Group>
        </Stack>
        ...
        {amenitiesList.length > 0 && (
          <Box mt="auto" pt="sm">
            <Text size="xs" fw={600} c="dimmed" mb={6}>AMENITIES</Text>
            <Group gap={4}>
              {amenitiesList.slice(0, 3).map((amenity, idx) => (
                <Badge key={idx} size="xs" variant="light" color="gray">{amenity}</Badge>
              ))}
              {amenitiesList.length > 3 && (
                <Badge size="xs" variant="light" color="blue">+{amenitiesList.length - 3} more</Badge>
              )}
            </Group>
          </Box>
        )}
