...
import { Modal, Stack, TextInput, Button, Group, Select, NumberInput, Paper, Text } from '@mantine/core';
...
export function ReservationFormModal({ opened, onClose, hotelId, reservationToEdit, guests, availableRooms }: Props) {
  ...
  useEffect(() => {
    ...
    // 1. Cari Harga Per Malam (Price Per Night)
    let pricePerNight = 0;

    // Coba cari di daftar kamar tersedia (availableRooms)
    ...
    // Jika tidak ketemu (misal sedang edit reservasi & kamarnya status occupied),
    // ambil harga dari data reservasi yang sedang diedit
  ...
  const roomOptions = useMemo(() => {
    ...
    // Jika sedang edit, pastikan kamar yang sedang dipakai juga muncul di opsi (walaupun statusnya occupied/tidak ada di availableRooms)
  ...
  return (
    <Modal opened={opened} onClose={onClose} title={reservationToEdit ? 'Edit Reservasi' : 'Reservasi Baru'} centered size="lg">
        ...
        <Stack gap="md">
          ...
          <Paper withBorder p="xs" bg="gray.0">
            <Group justify="space-between">
              <Text size="sm" fw={500}>Total Harga:</Text>
              <Text size="lg" fw={700} c="teal">{calculatedPrice ? `Rp ${calculatedPrice.toLocaleString('id-ID')}` : '-'}</Text>
            </Group>
          </Paper>
