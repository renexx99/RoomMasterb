...
import { StaffMember } from '../client';

interface Props {
  opened: boolean;
  onClose: () => void;
  itemToEdit: StaffMember | null;
  hotelId: string | null;
  roles: { label: string; value: string }[];
}

export function UserFormModal({ opened, onClose, itemToEdit, hotelId, roles }: Props) {
  ...
  useEffect(() => {
    if (opened) {
      if (itemToEdit) {
        const activeAssignment = itemToEdit.assignment;
  ...
  const handleSubmit = async (values: UserFormData) => {
    ...
    try {
      const payload = { ...values, hotel_id: hotelId || undefined };
      let result;
      if (itemToEdit) {
        result = await updateUserAction(itemToEdit.id, payload);
      } else {
  ...
  return (
    <Modal 
        opened={opened} 
        onClose={onClose} 
        title={<Text fw={700}>{itemToEdit ? 'Edit Staff Details' : 'Add New Staff'}</Text>} 
        centered
        radius="md"
    >
